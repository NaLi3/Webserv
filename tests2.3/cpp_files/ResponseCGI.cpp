...
char**	Response::createEnvArray(std::string& fullPath, int* envArraySize)
{
	...
	for (envIt = env.begin(); envIt != env.end(); envIt++)
	{
		...
		envVar = new char[envVarStr.size() + 1];
		bzero(envVar, envVarStr.size() + 1);
		strcpy(envVar, envVarStr.c_str());
		envArray[ind] = envVar;
		ind++;
	}
...
void	Response::childExecuteCGI(std::string& fullPath, int fdInput, int fdOutput)
{
	...
	argv[0] = new char[fullPath.size() + 1];
	bzero(argv[0], fullPath.size() + 1);
	strcpy(argv[0], fullPath.c_str());
	argv[1] = NULL;
