...
// First checks the validity of headers ("Content-Length" absent, "Content-Type" present)
// and defines default status code and description if not provided in CGI output,
// then concatenates the status line + all header lines + the CGI output body (if there is one)
...
// 	   even if they are logged with function <logError>, they won't stop the server,
// 			they will only make the server return an error response to the client
...
int	Response::processCGIOutput()
{
	...
	if (this->parseCGIHeaders(cgiHeaders, statusCode, statusDesc))
		return (this->makeErrorResponse(502), 0); // #f : check which errcode to use for "invalid CGI output"
	...
	if (cgiHeaders.count("Location") == 1)
	{
		if (cgiHeaders["Location"].size() == 0)
			return (this->makeErrorResponse(502), 0);
...
int	Response::handleCGI()
{
	...
	if (this->pathToResource(fullPath, 1, 1, NULL, 0))
		return (0);
