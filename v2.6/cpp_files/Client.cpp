...
// When the client instance is waiting for a CGI script to terminate and receives a POLLOUT event,
// checks if the script did terminate by calling <Response.checkCGIFinished>,
// then processes its return value exactly like in <handleCompleteRequest>
// 		(except if the CGI script terminated,
// 		 it is possible to send immediately after passing to state SENDING)
int	Client::updateWaitingCGI()
{
	...
	{
		std::cout << "\t... still waiting, no state change";
		return (0);
	}
	std::cout << "\t... CGI did terminate, pass to sending\n";
	this->passToSending();
...
// Special case : <Client> instance may be waiting for a CGI script managed by its response to terminate,
// 				  in this case it expects a POLLOUT to be able to check if its response is ready
...
{
	if ((pfd.revents & POLLOUT) && this->_state == WAITING_CGI)
...
int	Client::handleCompleteRequest()
{
	...
	std::cout << " -> Response instance returned successfully\n";
	this->passToSending();
	return (0);
}
